...
| Skill                       | What it does                                                                                                                                                                      | <br />                                                                                                                                                                                                                           |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `plan`                      | Architecture decomposition — reads a spec, produces `arch.json` with components, protocols, wiring, and composition root                                                          | <br />                                                                                                                                                                                                                           |
| `apply-principle-review`    | Single-principle review — reads rule.md, applies metrics, produces findings                                                                                                       | <br />                                                                                                                                                                                                                           |
| `synthesize-fixes`          | Holistic fix planner — sees ALL findings, cross-checks every fix against ALL principles, produces unified plan per file                                                           | <br />                                                                                                                                                                                                                           |
| `create-type`               | Enforces naming conventions and file organization when creating new types                                                                                                         | <br />                                                                                                                                                                                                                           |
| `prepare-review-input`      | Normalizes input (branch, folder, files) into structured `review-input.json`                                                                                                      | <br />                                                                                                                                                                                                                           |
| `validate-findings`         | Filters findings to changed code only, reorganizes outputs by file                                                                                                                | <br />                                                                                                                                                                                                                           |
| `find-spec`                 | Navigates spec hierarchy interactively, returns selected spec. Used by build-spec                                                                                                 | <br />                                                                                                                                                                                                                           |
| `validate-spec`             | Checks a spec for buildability — flags vague terms, undefined types, implicit contracts. Used by build-spec Phase 4                                                               | <br />                                                                                                                                                                                                                           |
| `validate-decomposition`    | Validates arch.json against SOLID principles — splits components, adds protocols, restructures hierarchies                                                                        | <br />                                                                                                                                                                                                                           |
| `reconstruct-spec`          | Reads arch.json ONLY (blind to original spec) and reconstructs what the architecture would deliver                                                                                | <br />                                                                                                                                                                                                                           |
| `validate-completeness`     | Compares reconstructed spec against original spec, diffs, adds missing components to arch.json                                                                                    | <br />                                                                                                                                                                                                                           |
| `validate-plan`             | Validates arch.json against the codebase — finds reusable types, conflicts, annotates components with reuse status                                                                | <br />                                                                                                                                                                                                                           |
| `synthesize-implementation` | Reconciles arch.json + validation.json into ordered implementation plan of /code directives                                                                                       | <br />                                                                                                                                                                                                                           |
| `validate-implementation`   | Post-code checkpoint — collects user screenshots and feedback, compares against design references, produces fix directives                                                        | <br />                                                                                                                                                                                                                           |
| `predict-loc-heuristic`     | Counts ACs + screens in a spec, applies the README § Scope Metrics formula, emits `heuristic.json`. Phase C measurement subagent.                                                 | <br />                                                                                                                                                                                                                           |
| `predict-loc-skeleton`      | Sketches declaration-only skeleton in the project's language (default Swift), counts LOC, applies a per-language multiplier, emits `skeleton.json`. Phase C measurement subagent. | <br />                                                                                                                                                                                                                           |
| `cohesion-cluster`          | Extracts 4 signals per AC, runs 2-signal transitive clustering, labels each group. Emits `cohesion.json`. Phase C measurement subagent.                                           | <br />                                                                                                                                                                                                                           |
| `scope-synthesize`          | Merges `heuristic.json` + `skeleton.json` + `cohesion.json` into `scope-assessment.json` (the Phase C public contract). Pure deterministic script.                                | <br />                                                                                                                                                                                                                           |
| v                           | `propose-split`                                                                                                                                                                   | When `verdict == needs_split`, reads spec + `scope-assessment.json` and produces `split-plan.json` with full per-candidate breakdown (TR partition, IO partition, Connects To partition, dependencies) and parent-level residue. |
...
| `predict-loc-heuristic-agent`     | Runs the heuristic LOC counter via Bash script (model: haiku)                                                                                    |
| `predict-loc-skeleton-agent`      | Sketches the spec into language-appropriate skeleton code and counts LOC (model: sonnet)                                                         |
| `cohesion-cluster-agent`          | Clusters spec ACs by 4-signal transitive rule and labels split candidates (model: sonnet)                                                        |
| `scope-synthesize-agent`          | Merges Phase C measurement outputs into `scope-assessment.json` via Bash script (model: haiku)                                                   |
| `propose-split-agent`             | Builds a full split plan (TR/IO/Connects-To partition, dependencies, parent residue) when validate-spec verdict is `needs_split` (model: sonnet) |
...
| ----------- | ----------------------------------- | ---------------------------------------------------------- |
| `README.md` | <br />                              | Common frontmatter fields, section rules, folder structure |
...
| Mode                                        | rule.md | code/instructions.md | fix/instructions.md | review/instructions.md | Examples | required\_patterns |
| ------------------------------------------- | :-----: | :------------------: | :-----------------: | :--------------------: | :------: | :----------------: |
