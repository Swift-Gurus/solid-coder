...
status: draft
parent: SPEC-010
blocked-by: [SPEC-013, SPEC-014, SPEC-028]
blocking: []
---
...
Prevents a subagent spawned to drive a delegated, isolated child flow (SPEC-028's `mode: subagent` delegate step) from ending its own turn while that isolated run is left `in_progress` with a step still pending — the `SubagentStop` counterpart to SPEC-014's main-agent `Stop` check, reusing the same turn-blocking and attempt-exhaustion mechanism, pointed at the subagent's own isolated run instead of the main run.
...
**Why this still matters despite SPEC-014 already covering the parent.** The parent's own turn-ending is already blocked correctly by SPEC-014 while the delegate step (from the parent's perspective, an ordinary pending step) remains unsubmitted — a parent can't silently walk away from a delegate step it never got a result for. What SPEC-014 does *not* catch: the subagent itself abandoning its isolated child flow mid-way and reporting back to the parent anyway (a plausible-looking but incomplete result), or simply leaving an orphaned isolated run directory behind. This spec closes that narrower gap; it is a safety net for the subagent's own diligence, not the safety-critical parent-side guarantee (which is already secure).
...
| Output | An allow/block decision through the same hook response protocol SPEC-014 uses. When blocking, the reason names the isolated flow, run, and pending step(s), matching SPEC-014's message shape for the main run. |
...
- This check never runs on a re-entrant stop attempt, matching SPEC-014's US-1 re-entrancy rule.

### US-3: Same attempt-exhaustion safety valve as the main run

As the system, when the same isolated-run pending step keeps blocking a subagent's turn-ending across repeated attempts, I want each block to count as one failed attempt against that step using the exact same mechanism SPEC-014 uses for the main run, so a stuck subagent isn't blocked forever either.

**Acceptance Criteria:**
- Each blocked subagent stop attempt consumes one attempt against the isolated run's pending step, through the identical shared attempts mechanism SPEC-014 and SPEC-027 already use — not a separate, isolated-run-specific counter.
...
- **Dispatch integration**: registers as one more independently-applicable check within the same generic Stop/SubagentStop dispatch mechanism SPEC-014 describes (concurrent evaluation, deny-wins aggregation, fail-open on internal error) — not a standalone script with its own response protocol.
...
| Upstream | SPEC-014 Agent Flow Stop Hook | Reuses SPEC-014's turn-blocking, attempt-exhaustion, and dispatch-integration mechanisms unchanged, applied to an isolated run instead of the main run. |
...
```mermaid
graph LR
  SPEC028[SPEC-028 Delegate Steps and Isolated Runs] --> SPEC015[SPEC-015 Subagent Flow Stop Hook]
  SPEC014[SPEC-014 Agent Flow Stop Hook] -.reused turn-blocking mechanism.-> SPEC015
...
- [ ] Each block consumes one attempt against the isolated run's pending step via the same shared mechanism SPEC-014 uses
