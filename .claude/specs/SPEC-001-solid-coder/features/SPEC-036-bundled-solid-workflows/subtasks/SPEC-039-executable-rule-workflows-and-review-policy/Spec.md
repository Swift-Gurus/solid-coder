...
- The bundled `workflows/review/rules/dry` package preserves the DRY-1/DRY-2/DRY-3 detection blocks and complete exception block. It reviews one immutable normalized unit, gives the canonical DRY-2 metric step the reviewed code directly, uses typed MCP-owned `source.search` and `source.read_candidates` operations for proposed sibling, proposed cross-file, and repository evidence, asks the model only for semantic terms and supplied-candidate classifications, and leaves scoring to MCP.
- Deterministic DRY flow coverage proves ordered agent/operation transitions, exact reviewed-unit exclusion, proposed-context precedence over stale disk paths, same-file and multi-file patch candidate retention, exact candidate-unit loading, engine-owned project-root/search/read execution, canonical instruction parity, exact MCP scoring, and continued within-unit DRY-2 evidence when source search returns no candidates.
...
- One unmarked nested DRY search workflow executes for the reviewed unit. It assembles the effective query through MCP, searches sibling units in the same source plus external repository units, loads exact candidate-unit slices through MCP, then fans out classification without allowing the model to discover files, choose roots, create identities, author exclusions, or call search/read tools.
...
- DRY search excludes the exact current normalized unit, retains sibling units from the same source, includes external repository units, and loads only the selected candidate unit rather than its complete containing file.
