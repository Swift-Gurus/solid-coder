...
- The bundled `code-smells`, `swiftui`, `structured-concurrency`, `unit-testing`, and `ui-testing` packages preserve their canonical detection procedures, authored exceptions, scalar schemas, severity bands, and typed applicability. All five use the shared deterministic parity/scoring/audit contract. `unit-testing` and `ui-testing` are file-scoped because their metrics inspect suite-wide structure, shared setup, lifecycle, and sibling declarations; each executes once per applicable normalized file rather than once per extracted unit.
...
- The bundled `unit-testing` and `ui-testing` rules declare `scope: file`. Their deterministic file tags select the applicable rule, and files containing multiple suites, helper types, or extensions still materialize exactly one rule instance with the complete prospective file snapshot.
...
- Swift unit-testing and UI-testing preserve file-wide semantics: `unit-testing` matches the `unit-test` file tag, `ui-testing` matches the `ui-test` file tag, and neither workflow performs a second unit-level fan-out after selection.
...
- A Swift test file containing multiple extracted declarations materializes exactly one `unit-testing` or `ui-testing` rule instance, supplies the normalized file identity to every rule step, and publishes one file-owned result; it never repeats the complete test review once per declaration.
...
- [x] SRP, OCP, LSP, ISP, DRY, code-smells, frontmatter, SwiftUI, structured-concurrency, unit-testing, and ui-testing each execute from one bundled rule workflow with preserved detection, exception, observation, schema, band, and activation semantics; unit-testing and ui-testing execute once per applicable file.
