...
parent: SPEC-010
blocked-by: [SPEC-030]
blocking: [SPEC-032, SPEC-033, SPEC-028, SPEC-034, SPEC-035, SPEC-036]
---
...
Four tools added to `mcp-server/pipeline/server.py` that expose the flow engine (SPEC-030) over MCP. Agents interact with the harness exclusively through these tools. The tools own the active run pointer and record session→step mappings needed by the stop hooks.
...
**Acceptance Criteria:**
- `flow_start(flow, params?)` resolves legacy flow YAML through the project/plugin harness roots; SPEC-035 supersedes first-match lookup with stable package IDs and collision-checked discovery while retaining these roots for compatibility
