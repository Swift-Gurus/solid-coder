...
## S-32 (DRY principle) — moved to archive

`references/principles/DRY/` exists with the full structure. See `improvements-archive.md`.

---

## S-33 (component discovery) — moved to archive

Superseded by `search_codebase` MCP tool in `mcp-server/pipeline/server.py`. See `improvements-archive.md`.
