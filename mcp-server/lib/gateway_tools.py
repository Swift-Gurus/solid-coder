        ...
        for file_obj in files:
            ...
            for unit in file_obj.get("units", []):
                ...
                for principle_name, principle_metrics in metrics_by_principle.items():
                    ...
                    # Flatten {var: {value: N, ...}} → {var: N} for scorer
                    flat: dict[str, Any] = {
                        var: entry["value"]
...
class PartialOutputValidator:
    ...
    Pass 1: validates overall structure against references/review-output.schema.json,
            stripping 'violations' from unit required since the server fills those.
    Pass 2: for each principle key found in unit metrics, validates the sub-object
            against that principle's review/output.schema.json (metrics slice schema).
    ...
    def validate_output(self, partial_output: dict) -> Optional[dict]:
        ...
        # Pass 1: unified structure schema
        ...
        # Pass 2: per-principle metrics schema
...
class SubmitOrchestrator:
    ...
    def orchestrate(self, partial_output: dict, output_path: str) -> dict[str, Any]:
        ...
        # Validate before scoring: schema errors are clearer than scoring evaluation errors
        ...
        err = self._submitter.submit(timestamp, scored_files, output_path)
        if err:
            return err
...
def _parse_principle_schema(schema_path) -> Optional[dict]:
    """Parse a per-principle metrics schema (describes only the metrics sub-object).

    Returns:
        {
            "metrics_example": dict  — {var_name: {"value": minimal_value}, ...}
        }
    or None when schema is absent or has no required metric variables.
    """
...
class PrincipleContentBuilder:
    ...
    def build(self, p_entry: dict) -> dict:
        ...
        if metrics_example:
            # Emit full payload shape so the LLM knows exactly what to submit
            payload_example = {
                "timestamp": "ISO-8601",
                "files": [{
                    "file_path": "...",
...
def _violation_key(rule_id: str, file_path: str, unit_name: str) -> str:
    """Stable identifier for a (rule_id, file, unit) triple used to match fixes to violations."""
...
class FixSubmitter:
    ...
    def submit_fix(self, output_dir: str, fixes: list) -> dict:
        ...
        for fix in fixes:
            try:
                key = _violation_key(fix["rule_id"], fix["file_path"], fix["unit_name"])
        ...
        violation_keys = {
            _violation_key(v["rule_id"], v["file_path"], v["unit_name"])
        ...
        if missing_keys:
            missing_ids = [
                ...
                if _violation_key(v["rule_id"], v["file_path"], v["unit_name"]) in missing_keys
            ]
            return {
                "error": (
        ...
        for fp in fixes_dir.glob("*.json"):
            try:
                ...
                fkey = _violation_key(data["rule_id"], data["file_path"], data["unit_name"])
...
class GatewayHandler:
    ...
    def submit_batch_findings(self, output_dir: str, submissions: dict) -> dict[str, Any]:
        ...
        Each entry in submissions maps a label to a partial_output using the new
        unified format: metrics keyed by principle name.
                ...
                for unit in file_obj.get("units", []):
                    ...
                    for violation in unit.get("violations", []):
                        if violation.get("severity") != "SEVERE":
                            continue
                        rule_id = violation.get("rule_id", "")
                        # Extract principle from rule_id prefix (e.g. "SRP-1" → "SRP")
                        ...
                        desc = measured or "SEVERE violation detected"
                        violations.append({
                            "rule_id": rule_id,
                            "file_path": file_path,
                            "unit_name": unit_name,
                            "issue": f"{rule_id}: {file_path}, unit {unit_name} — {desc}",
                            "fix": (
                                f"Call mcp__docs__load_fix_for_violation({rule_id}) "
