...
"""
solid-name: ValidateJsonSchemaCLITests
solid-category: unit-test
solid-description: Verifies the schema-validation hook CLI accepts valid writes and blocks malformed structured artifacts.
"""
class ValidateJsonSchemaCLITests(unittest.TestCase):
    ...
    def test_non_write_tool_is_allowed(self):
        rc, stdout, _ = _run(
            _payload("/tmp/arch.json", json.dumps(_VALID_ARCH), tool="Edit")
        )
        self.assertEqual(rc, 0)
        self.assertFalse(_is_blocked(stdout))

    def test_valid_arch_is_allowed(self):
        rc, stdout, _ = _run(_payload("/tmp/arch.json", json.dumps(_VALID_ARCH)))
        self.assertEqual(rc, 0)
        self.assertFalse(_is_blocked(stdout))

    def test_valid_validation_is_allowed(self):
        rc, stdout, _ = _run(
            _payload("/tmp/validation.json", json.dumps(_VALID_VALIDATION))
        )
        self.assertEqual(rc, 0)
        self.assertFalse(_is_blocked(stdout))
    ...
    def test_valid_plan_is_allowed(self):
        rc, stdout, _ = _run(
            _payload("/tmp/implementation-plan.json", json.dumps(_VALID_PLAN))
        )
        self.assertEqual(rc, 0)
        self.assertFalse(_is_blocked(stdout))
    ...
    def test_arch_missing_required_field_is_blocked(self):
        ...
        rc, stdout, _ = _run(_payload("/tmp/arch.json", json.dumps(bad)))
        self.assertEqual(rc, 0)
    ...
    def test_validation_missing_summary_is_blocked(self):
        ...
        del bad["summary"]
        rc, stdout, _ = _run(_payload("/tmp/validation.json", json.dumps(bad)))
        self.assertEqual(rc, 0)
        self.assertTrue(_is_blocked(stdout))
    ...
    def test_plan_missing_reconciliation_decisions_is_blocked(self):
        ...
        rc, stdout, _ = _run(
            _payload("/tmp/implementation-plan.json", json.dumps(bad))
        )
        self.assertEqual(rc, 0)
        self.assertTrue(_is_blocked(stdout))
