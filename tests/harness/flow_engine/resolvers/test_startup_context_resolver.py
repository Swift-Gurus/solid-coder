...
solid-spec: [SPEC-013]
solid-description: Tests gathering detected environment, base run directory, and search paths into one context.
"""
...
class TestStartupContextResolver(unittest.TestCase):
    ...
    def test_resolves_env_base_dir_and_stringified_search_paths(self):
        base_dir = Path("/runs")
        search_paths = [Path("/project/.solid-coder/harness/flows"), Path("/plugin/harness/flows")]
        sut = StartupContextResolver(
            env_detector=StubEnvDetector("claude_code"),
        ...
        self.assertEqual(context.detected_env, "claude_code")
