...
solid-spec: [SPEC-027]
solid-description: Tests building StepDef instances from raw step dictionaries, including type, script, and attempts fields.
"""
...
class TestStepBuilder(unittest.TestCase):
    ...
    def test_defaults_type_to_agent(self):
        step = self.sut.build({"id": "a", "prompt": "p"})
    ...
    def test_reads_explicit_max_attempts(self):
        step = self.sut.build({"id": "a", "prompt": "p", "max_attempts": 5})
        self.assertEqual(step.max_attempts, 5)

    def test_reads_script_type_command_and_timeout(self):
        step = self.sut.build({
            "id": "a", "type": "script", "command": ["run.sh", "arg"], "timeout_seconds": 30,
        })
    ...
    def test_reads_prompt_file_reference(self):
        step = self.sut.build({"id": "a", "prompt_file": "prompt.md"})
