...
- **Spec context flows through arch.json** — `acceptance_criteria[]`, `design_references[]`, and `design_decisions[]` are read from `arch.json` and embedded into individual plan item directives. Each directive gets only the criteria, designs, and decisions relevant to its component. For `modify` actions, only `unsatisfied_criteria[]` from `validation.json` are included (criteria already met by existing code are excluded).
