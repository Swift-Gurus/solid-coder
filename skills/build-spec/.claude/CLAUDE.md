...
- **Folder-per-epic hierarchy** — every epic creates a folder. Features, bugs, and subtasks are placed inside their parent epic's folder under `features/`, `features/bugs/`, or `features/subtasks/`. Epics can nest: a child epic gets its own subfolder inside the parent epic's folder. This mirrors the logical ownership structure in the filesystem.
